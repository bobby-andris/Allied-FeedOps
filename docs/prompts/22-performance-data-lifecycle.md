...
**Before starting performance investigation, verify the Search Insights sync is operational.**
...
The sync flow:
1. Dashboard calls `/api/search-insights/sync` (Vercel)
2. Vercel routes to Cloud Run `/search-insights/sync`
3. Cloud Run's `SearchTermsClient` uses Google Ads API
4. Results stored in `search_queries` and `search_queries_by_master_sku` tables

**Files Involved:**
- `src/feedops/integrations/google_ads_search_terms.py` - `SearchTermsClient` class
- `src/feedops/api/search_insights.py` - FastAPI router
- `dashboard/src/app/api/search-insights/sync/route.ts` - Dashboard proxy
