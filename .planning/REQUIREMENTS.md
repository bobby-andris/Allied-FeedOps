...
- [x] **TIER-04**: User can see hierarchical fallback scoring when per-group data is sparse (per-group → global → sensible defaults)
- [x] **TIER-05**: User can see "Insufficient data" degraded state when a tier has fewer than 5 terms with non-zero metrics
- [x] **TIER-06**: User can view confidence scores based on data volume, consistency, statistical significance, and NLP intent alignment
...
- [x] **FIX-02**: Under-invested detection compares avgMonthlySearches against actual totalImpressions (not boolean > 2)
...
- [x] **FIX-05**: Constrain button hidden when term is already in HIGH tier (no-op guard)
...
- [x] **FIX-07**: ROAS recommendations compare against tier distribution, not bid target
- [x] **FIX-08**: estimateTierFromMetrics() has wasted-spend guard — never returns LOW for zero-conversion terms
- [x] **FIX-09**: Scoring systems unified — recommendations API uses distribution-based scoring
...
- [ ] **INTENT-01**: Eradicate "constrain" terminology — replace with "demote" across all types, logic, UI components, tests, and verdict text
...
- [ ] **INTENT-05**: Fix tier-scoring UI — add targetTier to TermScore type, fix TierMovementArrow direction, show Promote/Demote buttons with target tier labels, display intent score breakdown (attribute + behavioral + composite)
...
| FIX-02 | Phase 34.1 | Complete |
| FIX-03 | Phase 34.1 | Complete |
| FIX-04 | Phase 34.1 | Complete |
| FIX-05 | Phase 34.1 | Complete |
