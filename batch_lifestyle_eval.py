...
async def batch_optimize():
    ...
    output_dir = "reports/lifestyle-eval"
    exports_dir = "exports/lifestyle-eval"
    ...
    for i, sku_info in enumerate(eval_skus, 1):
        ...
        try:
            ...
            if result.candidate.lifestyle_images:
                num_images = sum(
                    1 for img in result.candidate.lifestyle_images 
    ...
    if results["success"]:
        avg_score = sum(r["quality_score"] for r in results["success"]) / len(results["success"])
        print(f"\nAverage Quality Score: {avg_score:.2f}%")
    
    # Save results summary
    summary_path = Path(output_dir) / f"batch-summary-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    ...
    print(f"  streamlit run src/feedops/quality/review_dashboard.py -- \\")
    print(f"    --candidate {exports_dir} \\")
    print(f"    --catalog \"data/catalog/Product Catalog.csv\"")
    print("=" * 80)
