...
class SKUData:
    ...
    baseline: dict[str, ExportContent] = field(
        default_factory=dict
    )  # platform -> content
    candidate: dict[str, ExportContent] = field(
        default_factory=dict
    )  # platform -> content
...
def load_exports_dir(exports_dir: Path | str) -> dict[str, dict[str, ExportContent]]:
    ...
    for prefix, platform in prefixes:
        for path in exports_dir.glob(f"{prefix}*.json"):
            ...
            if selected_lifestyle_image is None and lifestyle_images:
                for img in lifestyle_images:
                    if isinstance(img, dict) and img.get("generation_success", False):
        ...
        for line in evidence_markdown.split("\n"):
            ...
            if len(parts) >= 4:
                ...
                if "keyword" in attr.lower():
                    keywords[attr] = value
                elif "enrichment" in source.lower() or attr.startswith(
                    (
                        "collection_",
                        "design_",
                        "feature_",
...
def load_all_sku_data(
    *,
    ...
    # Load scores
    baseline_scores = {
        row["sku"]: row for row in evaluate_exports_dir(Path(baseline_exports_dir))
    }
    candidate_scores = {
        row["sku"]: row for row in evaluate_exports_dir(Path(candidate_exports_dir))
    }
    ...
    all_skus = sorted(set(baseline_exports.keys()) | set(candidate_exports.keys()))
    ...
    for sku in all_skus:
        ...
        delta = (
            round(c_composite - b_composite, 2) if b_composite and c_composite else 0.0
        )
...
def get_summary_stats(sku_data: list[SKUData]) -> dict[str, Any]:
    ...
    avg_baseline = (
        round(sum(baseline_scores) / len(baseline_scores), 2)
        if baseline_scores
        else 0.0
    )
    avg_candidate = (
