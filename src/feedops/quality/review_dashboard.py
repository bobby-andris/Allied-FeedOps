...
def run_dashboard(
    ...
    if not all_sku_data:
        st.error("No SKU data found. Check that export directories exist and contain data.")
        return
    ...
    with st.sidebar:
        ...
        score_filter = st.selectbox(
            "Score Change",
            ["All", "Improved", "Declined", "Unchanged"]
        )
...
def filter_sku_data(
    ...
    if search_query:
        ...
        filtered = [
            d for d in filtered
            if query_lower in d.sku.lower()
        ]
    
    if category:
        filtered = [
            d for d in filtered
            if d.original and d.original.category == category
        ]
    
    if collection:
        filtered = [
            d for d in filtered
            if d.original and d.original.collection == collection
        ]
...
def render_reasoning_panel(sku_data: SKUData) -> None:
    ...
    # Provider info
    st.caption(f"Model: {report.provider_model or 'Unknown'} | Cost: {report.estimated_cost or 'N/A'}")
...
def render_score_panel(sku_data: SKUData) -> None:
    ...
    for dim_key, display_name in dimensions:
        ...
        chart_data.append({"Dimension": display_name, "Baseline": b_avg, "Candidate": c_avg})
...
def parse_args():
    ...
    parser.add_argument("--baseline", "-b", default="exports", help="Baseline exports directory")
    parser.add_argument("--candidate", "-c", default="exports/enriched-v2", help="Candidate exports directory")
    parser.add_argument("--catalog", default=None, help="Path to Product Catalog.csv")
    parser.add_argument("--baseline-reports", default=None, help="Baseline reports directory")
    parser.add_argument("--candidate-reports", default=None, help="Candidate reports directory")
    ...
    args.baseline_reports = os.environ.get("FEEDOPS_BASELINE_REPORTS", args.baseline_reports)
    args.candidate_reports = os.environ.get("FEEDOPS_CANDIDATE_REPORTS", args.candidate_reports)
...
def main():
    ...
    catalog_path = Path(args.catalog) if args.catalog else Path("data/catalog/Product Catalog.csv")
    baseline_reports = Path(args.baseline_reports) if args.baseline_reports else Path("reports")
    candidate_reports = Path(args.candidate_reports) if args.candidate_reports else Path("reports/enriched-v2")
