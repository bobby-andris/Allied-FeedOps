"""Route handlers for the FeedOps Pipeline API.

All HTTP endpoint handlers formerly in main.py live here, registered on a
single APIRouter that main.py mounts via ``app.include_router(main_router)``.

Extracted from main.py as part of DECOMP-09 (Plan 03-02).
"""
...
# API version — kept in sync with main.py via import at mount time
API_VERSION = "1.0.0"
...
@router.post("/optimize-sku", response_model=OptimizeResponse, tags=["Generation"])
...
@router.post("/hybrid-generate", response_model=HybridJobResponse, tags=["Generation"])
...
@router.get("/backfill/validation-report", response_model=ValidationReportResponse)
...
@router.get("/backfill/status/{job_id}", response_model=BackfillJobResponse)
async def api_get_backfill_status(job_id: str):
    """Get backfill job status and progress."""
    return await get_backfill_status(job_id)


@router.post("/backfill/resume/{job_id}", response_model=BackfillJobResponse)
