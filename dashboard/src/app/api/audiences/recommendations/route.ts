...
export async function GET(request: NextRequest) {
  try {
    ...
    const recommendations = buildAudienceRecommendations(watchItems, attribution)

    return NextResponse.json({
      generated_at: new Date().toISOString(),
      attribution_quality: attribution,
      recommendations,
      watchlist_snapshot: watchItems,
      warnings,
      available: warnings.length === 0,
    })
  } catch (error) {
    console.error('Audience recommendations fetch failed:', error)
    return NextResponse.json(
